...
- `/emoji <表情或ID>`：超级管理员给本条消息贴同款 QQ 表情；如果引用某条消息，则只给被引用的消息贴表情。`/emoji 368` 这类数字会直接作为 NapCat 表情 ID 尝试贴上，不可用时返回“表情不可用”。超级管理员单独发送一个商城/超级表情，或手动给某条消息贴表情时，机器人会尝试给同一条消息贴同款表情；失败时静默。`/emoji <表情>=<ID>` 可以绑定 Unicode 表情与贴表情 ID，`/emoji <表情>!=<ID>` 删除绑定。
- `/emojilist`：超级管理员查看当前已绑定的 Unicode 表情，记录保存在 `data/emoji_reactions/bindings.json`。不会自动学习 QQ 表情。
